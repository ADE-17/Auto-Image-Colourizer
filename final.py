...
if not os.path.isfile(model):
    sg.popup_scrolled('Missing model file', 'You are missing the file "colorization_release_v2.caffemodel"',
                      'Download it and place into your "model" folder', 'Model file missing:\n')
    exit()
...
left_col = [[sg.Text('Folder'), sg.In(size=(25,1), enable_events=True ,key='-FOLDER-'), sg.FolderBrowse()],
            ...
            [sg.CBox('Also show grayscale images',key='-MAKEGRAY-')],
            [sg.Text('EDI Project ' + version, font='Courier 8')]]

images_col = [[sg.Text('Input file:'), sg.In(enable_events=True, key='-IN FILE-'), sg.FileBrowse()],
              [sg.Button('Colorize Photo', key='-PHOTO-'), sg.Button('RESET', key='-WEBCAM-'), sg.Button('Save File', key='-SAVE-'), sg.Button('Exit')],
              [sg.Image(filename='', key='-IN-'), sg.Image(filename='', key='-OUT-')],]
# ----- Full layout -----
layout = [[sg.Column(left_col), sg.VSeperator(), sg.Column(images_col)]]

# ----- Make the window -----
window = sg.Window('AUTO-IMAGE COLOURIZER', layout, grab_anywhere=True)
    ...
    if event == '-FOLDER-':         # Folder name was filled in, make a list of files in the folder
        ...
        if filename != prev_filename:
            ...
            try:
                ...
                continue
#Web cam try kiya tha..but too large resolution
